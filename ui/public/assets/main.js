...
import { initialState, reduceDashboard } from './state/dashboardMachine.js';
...
function dispatch(action) {
    ...
    if (action.type === 'SET_AUTO_REFRESH' || action.type === 'SET_REFRESH_INTERVAL')
        configureAutoRefresh();
    render();
}
function uniqueSorted(values) {
    return [...new Set(values.map((value) => value?.trim()).filter((value) => Boolean(value)))].sort((a, b) => a.localeCompare(b));
}
function sensorUnit(sensor, fallback) {
    const units = {
        temperature: '°C',
        humidity: '%',
        pressure: 'hPa',
...
function allKnownMeasurements() {
    return [...recentMeasurements, ...historyMeasurements, ...(latestMeasurement ? [latestMeasurement] : [])];
}
function connectionLabel() {
    return { idle: 'nie testowano', checking: 'sprawdzanie…', online: 'online', offline: 'offline' }[state.connectionState];
}
...
function renderChart(measurements) {
    ...
    if (!data.length)
        return '<div class="empty-state">Brak historii dla podanych filtrów.</div>';
    const width = 900;
    const height = 320;
...
function renderLatest() {
    if (latestError)
        return `<div class="empty-state empty-state--error">${escapeHtml(latestError)}</div>`;
    if (!latestMeasurement)
        return '<div class="empty-state">Brak danych. Użyj „Pobierz latest” albo włącz auto-refresh.</div>';
...
function bindEvents() {
    byId('apiUrl').addEventListener('change', (event) => dispatch({ type: 'SET_API_URL', value: event.target.value }));
    byId('deviceId').addEventListener('change', (event) => dispatch({ type: 'SET_DEVICE_ID', value: event.target.value }));
    byId('sensor').addEventListener('change', (event) => dispatch({ type: 'SET_SENSOR', value: event.target.value }));
    byId('limit').addEventListener('change', (event) => dispatch({ type: 'SET_LIMIT', value: Number(event.target.value) }));
    byId('refreshInterval').addEventListener('change', (event) => dispatch({ type: 'SET_REFRESH_INTERVAL', value: Number(event.target.value) }));
    byId('autoRefresh').addEventListener('change', (event) => dispatch({ type: 'SET_AUTO_REFRESH', value: event.target.checked }));
    byId('testApi').addEventListener('click', testApi);
    byId('refreshAll').addEventListener('click', refreshAll);
    byId('fetchLatest').addEventListener('click', fetchLatest);
    byId('fetchHistory').addEventListener('click', fetchHistory);
    byId('clearMessages').addEventListener('click', () => dispatch({ type: 'CLEAR_MESSAGES' }));
}
function byId(id) {
...
async function getLatestForCurrentFilters() {
